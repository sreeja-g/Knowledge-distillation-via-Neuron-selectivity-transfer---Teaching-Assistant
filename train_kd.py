...
def main():
	...
	for epoch in range(1, args.epochs+1):
		...
		test_top1, _ = test(test_loader, nets, criterions, epoch)
...
def train_init(train_loader, nets, optimizer, criterions, total_epoch):
	...
	for epoch in range(1, total_epoch+1):
		...
		for i, (img, target) in enumerate(train_loader, start=1):
			...
			prec1, _ = accuracy(out_s, target, topk=(1,2))
...
def test(test_loader, nets, criterions, epoch):
	...
	f_l = [cls_losses.avg, kd_losses.avg, top1.avg]
	logging.info('Cls: {:.4f}, KD: {:.4f}, Prec@1: {:.2f}'.format(*f_l))
